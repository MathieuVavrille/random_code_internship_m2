...
main:
  | cstr           { let cstrset = $1 in
		     ...
		     List.fold_left (fun acc (x,y) -> if Strset.mem x !sbox_inactive then Cstrset.add (Iscst(y,0)) acc else acc) (Cstrset.add (ActiveSB(active_sbox_list, cstr_bound)) cstrset) !all_sbox, List.rev (List.fold_left (fun acc (a,b) -> a::b::acc) [] !all_sbox), cstr_bound }
...
cstr:
  ...
  | ZERO LPAREN VAR RPAREN SEMIC cstr { sbox_inactive := Strset.add $3 !sbox_inactive;Cstrset.add (Iscst($3,0)) $6 }
